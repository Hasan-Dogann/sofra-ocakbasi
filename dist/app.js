...
window.addEventListener('load',()=>{const extra=document.querySelector('.reviews-more .reviews-track');const main=document.querySelector('.reviews-section:not(.reviews-more) .reviews-track');if(extra&&main){[...extra.children].forEach(card=>main.appendChild(card));extra.parentElement.remove()}});

// Ana sayfadaki Google yorumlarını yumuşak bir döngüyle kaydır.
(()=>{const track=document.querySelector('[data-review-track]');if(!track)return;let timer;const start=()=>{clearInterval(timer);if(!matchMedia('(prefers-reduced-motion: reduce)').matches)timer=setInterval(()=>{const card=track.querySelector('.review-card');const step=card?card.getBoundingClientRect().width+20:0;if(!step)return;const end=track.scrollWidth-track.clientWidth;track.scrollTo({left:track.scrollLeft+step>=end-4?0:track.scrollLeft+step,behavior:'smooth'})},2500)};track.addEventListener('mouseenter',()=>clearInterval(timer));track.addEventListener('mouseleave',start);track.addEventListener('touchstart',()=>clearInterval(timer),{passive:true});track.addEventListener('touchend',start,{passive:true});start()})();
